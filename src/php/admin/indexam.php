   ...
   <div class="p-4">
      <div class="grid grid-cols-1 gap-4">
         ...
         <div class="w-full text-center bg-neutral-primary-soft rounded-full shadow-xs bg-gray-200 dark:bg-gray-700">
            <p class="mb-6 mt-6 text-base text-body sm:text-lg align-center text-red-400">
               ตารางสมุดประวัติที่สามารถสแกนเก็บไฟล์เอกสาร</p>
         </div>
      ...
      <div class="grid grid-cols-1 gap-4 p-4">
         ...
         <div class="w-full text-center bg-neutral-primary-soft shadow-xs bg-gray-200 dark:bg-gray-700 rounded-2xl">
            ...
            <!-- ในกรอบตาราง -->
            <div class="relative overflow-x-auto bg-neutral-primary-soft shadow-xs rounded-base mt-4 mb-4">
               ...
               /* สำคัญที่สุด */
               if (is_array($uData)) {
                  // ดึงข้อมูลจากตาราง k (ใน DB นี้)
                  $result = $conn->query("SELECT idcard, idnumber FROM loginfrom");

                  // เก็บ key ที่มีอยู่แล้ว
                  $kIds = [];
                  while ($row = $result->fetch_assoc()) {
                     $key = $row['idcard'] ?: $row['idnumber'];
                     if ($key) {
                        $kIds[$key] = true;
                     }
                  }
                  ...
                  <table class="w-full text-sm rtl:text-right text-body dark:text-white text-center">
                     ...
                     <tbody>
                        ...
                        foreach ($uData as $row) {
                           ...
                           // ❌ ถ้ามีอยู่ในตาราง k แล้ว → ไม่แสดง
                           if ($key && isset($kIds[$key])) {
                              continue;
                           }
                           ...
                           <tr class="odd:bg-neutral-primary even:bg-neutral-secondary-soft border-b border-default ">
                              ...
                              <td class="px-6 py-4">
                                 <button type="button" data-modal-target="book-history" data-modal-toggle="book-history"
      ...
      <div id="book-history" data-modal-backdrop="static" tabindex="-1" aria-hidden="true"
         ...
         <div class="relative p-4 w-full max-w-2xl max-h-full">
            ...
            <div class="relative bg-white rounded-2xl shadow-sm p-4 md:p-6 dark:bg-gray-800">
               <!-- Modal header -->
               <div class="flex items-center justify-between border-b border-default pb-4 md:pb-5 dark:border-gray-100">
               ...
               <form class="mb-2 mt-2" action="includes/server/bookhistorydb.php" method="post" id="bookhistoryForm">
                  ...
                  <div class="space-y-4 md:space-y-6 py-4 md:py-6">
                     <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 p-4">
                        ...
                        <div>
                           ...
                           <span id="IdnumberError" class="text-red-500"></span>
                        </div>


                        <div>
                           <label for="pfname_input" class="block font-medium text-red-400 text-md">
                              คำนำหน้าชื่อ
                           </label>
                           <div class="mt-2 mb-2">
                              <input id="pfname_input" type="text" name="pfname"
                                 class="w-full rounded-4xl px-4 py-4 text-base text-gray-900 dark:text-gray-100 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6 bg-gray-200 dark:bg-gray-700" />
                           </div>
                           <span id="pfnameError" class="text-red-500"></span>
                        </div>

                        <div>
                           <label for="firstname_input" class="block font-medium text-red-400 text-md">
                              ชื่อ
                           </label>
                           <div class="mt-2 mb-2">
                              <input id="firstname_input" type="text" name="name"
                                 class="w-full rounded-4xl px-4 py-4 text-base text-gray-900 dark:text-gray-100 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6 bg-gray-200 dark:bg-gray-700" />
                           </div>
                        ...
                        <div>
                           <label for="midname_input" class="block font-medium text-red-400 text-md">
                              ชื่อกลาง
                           </label>
                           <div class="mt-2 mb-2">
                              <input id="midname_input" type="text" name="midname"
                                 class="w-full rounded-4xl px-4 py-4 text-base text-gray-900 dark:text-gray-100 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6 bg-gray-200 dark:bg-gray-700" />
                           </div>
                           <span id="midnameError" class="text-red-500"></span>
                        </div>

                        <div>
                           <label for="lastname_input" class="block font-medium text-red-400 text-md">
                              นามสกุล
                           </label>
                           <div class="mt-2 mb-2">
                              <input id="lastname_input" type="text" name="lastname"
                                 class="w-full rounded-4xl px-4 py-4 text-base text-gray-900 dark:text-gray-100 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6 bg-gray-200 dark:bg-gray-700" />
                           </div>
                        ...
                        <div>
                           ...
                           <div class="mt-2 mb-2">
                              <input id="date_input" type="date" name="date"
                        ...
                        <div>
                           ...
                           <div class="mt-2 mb-2">
                              <fieldset class="w-full rounded-4xl px-4 py-4 bg-gray-200 dark:bg-gray-700 sm:text-sm/6">
                  ...
                  <div class="space-y-4 md:space-y-6 py-4 md:py-6">
                     <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 p-4">
                        ...
                        <div>
                           <label for="note_input" class="block font-medium text-red-400 text-md">
                              หมายเหตุ
                           </label>
                           <div class="mt-2 mb-2">
                              <input id="note_input" type="text" name="note"
                                 class="w-full rounded-4xl px-4 py-4 text-base text-gray-900 dark:text-gray-100 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6 bg-gray-200 dark:bg-gray-700" />
                           </div>
                        </div>
                        ...
                        <div>
                           <label for="filenumber_input" class="block font-medium text-red-400 text-md">
                              แฟ้มข้อมูลในการเก็บไฟล์
                           </label>
                           <?php
                           // ดึง filenumber จากตาราง finfor
                           $result = $conn->query("SELECT filenumber FROM finfor");
                           ?>

                           <div class="mt-2 mb-2">
                              <select id="filenumber_input" name="filenumber" class="w-full rounded-4xl px-4 py-4 text-base text-gray-900 dark:text-gray-100
                   focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500
                   sm:text-sm/6 bg-gray-200 dark:bg-gray-700">

                                 <option disabled selected>กรุณาเลือกแฟ้มข้อมูล</option>

                                 <?php while ($row = $result->fetch_assoc()) { ?>
                                    <option value="<?= htmlspecialchars($row['filenumber']) ?>">
                                       <?= htmlspecialchars($row['filenumber']) ?>
                                    </option>
                                 <?php } ?>
                        ...
                        <div id="upload-box" class="hidden mt-4">
                           <label class="block font-medium text-red-400 text-md"
                              for="file_input">อัพโหลดไฟล์สมุดประวัติ</label>
                           <div class="mt-2 mb-2">
                              <input
                                 class="cursor-pointer w-full rounded-4xl px-4 py-4 text-base text-gray-900 dark:text-gray-100 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6 bg-gray-200 dark:bg-gray-700"
                     ...
                     <script>
                        document.getElementById('filenumber_input').addEventListener('change', function () {
      ...
      <script>
         ...
         function loadRowData(idlogin) {
            fetch('http://localhost/kpb-profilebook_api/get_userpdx_button.php?idlogin=' + encodeURIComponent(idlogin))
               .then(res => res.json())
               .then(data => {
                  console.log(data);

                  if (data.status === 'exists') {
                     alert('ข้อมูลนี้ถูกใช้แล้ว');
                     return;
                  }
                  ...
                  document.getElementById('pfname_input').value = data.pfname ?? '-';
                  document.getElementById('firstname_input').value = data.firstname ?? '-';
                  document.getElementById('midname_input').value = data.midname ?? '-';
                  document.getElementById('lastname_input').value = data.lastname ?? '-';
